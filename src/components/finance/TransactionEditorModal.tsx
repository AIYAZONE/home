    ...
    <>
      ...
      <div className="fixed inset-0 z-[9999] flex items-end justify-center bg-black/40 p-4 backdrop-blur-sm sm:items-center" onClick={props.onClose}>
        <div className="w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
          ...
          <CardContent className="max-h-[calc(100vh-10rem)] overflow-hidden">
            <form
              onSubmit={(e) => {
                e.preventDefault();
            ...
            >
                ...
                {props.mode === 'add' && latestTransaction && (
                  <div className="flex flex-col gap-2 rounded-2xl border border-border/60 bg-surface-2 px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
                    <div className="text-xs text-muted-foreground">
                      最近：{formatMoney(latestTransaction.type === 'income' ? latestTransaction.amount : -latestTransaction.amount, { signDisplay: 'always' })} ·{' '}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  {recommendedCategories.length > 0 && (
                    <div className="mb-2 flex min-w-0 items-center gap-2">
                      <div className="shrink-0 text-xs text-muted-foreground">推荐</div>
                        ...
                        {recommendedCategories.map((name) => (
                          <button
                            ...
                            className={cn(
                              ...
                              name === category ? 'border-primary/30 bg-primary/10' : 'border-border bg-card',
                            )}
                    ...
                    <div className="mt-2 flex gap-2 overflow-x-auto py-0.5">
                      {quickCategories.slice(0, 8).map((c) => (
                        <button
                          ...
                          className={cn(
                            'shrink-0 rounded-full border px-3 py-1 text-xs font-medium',
                            c.name === category ? 'border-primary/30 bg-primary/10' : 'border-border bg-card',
                          )}
                ...
                <CollapsibleCard
                  title="更多选项"
                  description="描述、固定支出/订阅等低频项"
                  className="border border-border/60 bg-surface-2"
                >
